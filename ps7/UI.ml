...
let mouse_motion : (int * int) Event.event = Event.new_event ()

(** Fires each time the virtual clock ticks. *)
let clock : unit Event.event = Event.new_event ()
...
let read_event () =
  ...
  if new_pos <> !mouse_pos then begin
    mouse_pos := new_pos ;
    Event.fire_event mouse_motion (Graphics.mouse_pos ())
  end ;
...
let run_world (init:unit -> unit) (clock_f:unit -> unit) : unit =
  run_ui (World.size*World.obj_width) (* GUI width *)
         ...
         begin fun () ->
           World.reset () ;
           ignore(Event.add_listener clock (clock_handler clock_f)) ;
