...
let melody = list_to_stream ((List.map quarter slow)@(List.map eigth fast));;
*)
...
let part3 = list_to_stream [Note((G,4),1.,60); Note((G,4),0.5,60); 
                            Note((E,3),0.1875,60); 
                            Note((Gb,3),0.0625,60); Note((G,4),0.25, 60);
                            Note((E,3),0.25,60)];;
